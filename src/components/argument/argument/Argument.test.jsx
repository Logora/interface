...
const httpClient = {
	...
	delete: (url, data, config) =>
		new Promise((resolve) => resolve({ data: { success: true, data: {} } })),
};
...
describe("Argument", () => {
	...
	it("should render argument with replies", () => {
		...
		const user1Img = getByAltText(
			`${argumentWithReplies.replies_authors[0].full_name}'s profile picture`,
		);
		const user2Img = getByAltText(
			`${argumentWithReplies.replies_authors[1].full_name}'s profile picture`,
		);
		const user3Img = getByAltText(
			`${argumentWithReplies.replies_authors[2].full_name}'s profile picture`,
		);
		expect(user1Img).toBeInTheDocument();
		expect(user2Img).toBeInTheDocument();
		expect(user3Img).toBeInTheDocument();
		expect(user1Img).toHaveAttribute(
			"src",
			argumentWithReplies.replies_authors[0].image_url,
		);
		expect(user2Img).toHaveAttribute(
			"src",
			argumentWithReplies.replies_authors[1].image_url,
		);
		expect(user3Img).toHaveAttribute(
			"src",
			argumentWithReplies.replies_authors[2].image_url,
		);
	});
	...
	it("should render argument as a reply", () => {
		...
		const argumentContainer = container.querySelector(
			`#argument_${argumentReply.id}`,
		);
	...
	it("should render deleted argument", () => {
		...
		expect(
			queryByText(argumentDeleted.author.full_name),
