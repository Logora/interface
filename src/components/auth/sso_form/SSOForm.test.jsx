...
describe("SSOForm", () => {
	it("should render with correct defaults", () => {
		const component = render(<DefaultSSOForm />);
		...
		expect(signinLink.href).toContain(
			"https://example.com/login?client_id=client-id",
		);
		expect(signinLink.href).toContain("response_type=code");
		expect(signinLink.href).toContain("state=");
		expect(signupButton.href).toContain(
			"https://example.com/signup?client_id=client-id",
		);
	...
	it("should render with correct links if not OAuth2", () => {
		const component = render(<SSOFormWithRedirect />);
		...
		expect(signinLink).toBeTruthy();
		expect(signinLink.href).toContain(
			"https://example.com/login?logora_redirect=",
		);
		expect(signupButton.href).toContain(
			"https://example.com/signup?logora_redirect=",
		);
		expect(decodeURIComponent(signinLink.href)).toContain(
			window.location.origin,
		);
		expect(decodeURIComponent(signupButton.href)).toContain(
			window.location.origin,
		);
		expect(screen.getByText("Already have an account ?")).toBeTruthy();
		expect(
	...
	it("should render with consent toggle when showEmailConsent is true", () => {
		const component = render(<SSOFormWithEmailConsent />);
