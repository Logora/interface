...
export const LineChart = ({
	...
	const CHART_OPTIONS = {
		...
		plugins: {
			title: { display: true, font: { size: "16" }, text: title },
	...
	const formatLineChartData = (data) => {
		const finalData = {
			labels: data[0].data
				.map((elm) => elm.dimension.split("-").reverse().join("-"))
				.map((elm, index) => (index % 3 === 0 ? elm : "")),
			...
			<div className={cx(styles.lineChartContainer, className)}>
				<div className={styles.lineChartFilterOptions}>
					<Select
						...
						onChange={(selectedObject) => onSetMetric(selectedObject.value)}
					/>
					{labels && (
						<div className={styles.filterTagList}>
							{[
								{
									id: null,
									display_name: intl.formatMessage({ id: "info.all" }),
									name: "aaa",
								},
								...labels,
							]
