...
describe("ConsultationBox", () => {
	it("renders ConsultationBox component", () => {
		const { getByText } = render(
			<DefaultConsultationBox consultation={consultation} />,
		);
	...
	it("renders a clickable title redirecting to the correct link", () => {
		...
		expect(getByText(consultation.title).closest("a")).toHaveAttribute(
			"href",
			"/espace-debat/consultation/" + consultation.slug,
		);
	});
