...
describe("EmbedHeader", () => {
	it("renders header label, title and link with correct href value", () => {
		...
		expect(screen.getByText(/Test Title/i).closest("a")).toHaveAttribute(
			"href",
			"/test-url",
		);
	});
	...
	it("renders online users count if provided", () => {
		...
		expect(getByTestId("online-users-count")).toHaveTextContent(
			`${onlineUsersCount} online users`,
		);
		expect(getByTestId("online-users-count-mobile")).toHaveTextContent(
