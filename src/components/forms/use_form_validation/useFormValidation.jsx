...
export const useFormValidation = () => {
	...
	const validate = (formData, validationRules) => {
		...
		validationRules.forEach((rule) => {
			const validationField = Object.keys(rule)[0];
			const validationMethod = Object.values(rule)[0][0];
			const validationChecker = Object.values(rule)[0][1];

			if (validationMethod === "required") {
				if (required(formData, validationField)) {
					errors[validationField] = intl.formatMessage({
						id: `errors.${validationField}_required`,
						defaultMessage: `${validationField} can't be empty.`,
					});
				}
			}
				...
				if (checkValue(formData[validationField], validationChecker)) {
					errors[validationField] = intl.formatMessage(
						...
						},
						{
							...
							validation_value: validationChecker,
						},
					);
				}
			}
			...
			if (validationMethod === "question") {
				if (isQuestion(formData[validationField])) {
					errors[validationField] = intl.formatMessage({
						id: `errors.${validationField}_question`,
						defaultMessage: `${validationField} must have a question mark at the end.`,
					});
				}
			}
			if (validationMethod === "url") {
				if (urlInText(formData[validationField])) {
					errors[validationField] = intl.formatMessage({
						id: `errors.${validationField}_url`,
						defaultMessage: `${validationField} must not contain any links`,
					});
				}
			}
