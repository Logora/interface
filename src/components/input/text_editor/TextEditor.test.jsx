...
describe("TextEditor", () => {
	it("should render with placeholder", () => {
		const editor = render(<DefaultTextEditor />);
	...
	it("should focus editor when setFocus called from outside", async () => {
		const FocusInputComponent = (props) => {
			...
			return <div onClick={focusInput}>Click to focus</div>;
		};
	...
	it("should set content in editor when setContent called from outside", async () => {
						...
						children: [
							{
								...
								mode: "normal",
								style: "",
								text: "Integer pretium varius odio ac eleifend.",
								type: "text",
								version: 1,
							},
						],
		...
		const SetInputContentComponent = (props) => {
			...
			return <div onClick={setRichContent}>Click to set content</div>;
		};
		...
		expect(
			container.getByText("Integer pretium varius odio ac eleifend."),
		).toBeTruthy();
	});
	...
	it("should display max length and update it", async () => {
		...
		const maxLengthUpdated = getByText("remaining characters", {
			exact: false,
		});
...
describe("AutoSavePlugin", () => {
	...
	it("should auto save content in local storage", async () => {
		...
		await act(async () => {
			userEvent.click(setContentButton);
		});
