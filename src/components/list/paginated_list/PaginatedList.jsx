...
export const PaginatedList = ({
	...
	const getInitSort = () => {
		return (
			(sortOptions && sortOptions[0].type === "sort" && sortOptions[0].value) ||
			sort ||
			""
		);
	};
	...
	const loadResources = (pageNumber) => {
		...
		if (
			(pageNumber - 1) * perPage <= (numberElements || totalElements) ||
			pageNumber === 1
		) {
			const params = {
			...
			loadFunction(resource, params)
				.then((response) => {
					const headers = response.headers;
					if (headers) {
						if (totalHeaderParam in headers) {
							setTotalElements(
								Number.parseInt(headers[totalHeaderParam || "total"], 10),
							);
							if (onUpdateTotal) {
					...
					if (
						list.addElements &&
						currentListId in list.addElements &&
						list.addElements[currentListId].length > 0
					) {
	...
	const handleRemoveElements = (elements) => {
		...
		const newElements = currentResources.filter(
			(elm) => removedElementsIds.indexOf(elm.id) === -1,
		);
		setCurrentResources(uniqueBy(newElements, uniqueIdKey || "id"));
	};
				...
				>
					<StandardErrorBoundary hideMessage={true}>
						{React.cloneElement(children, {
							...{ index: index, [resourcePropName]: resource },
						})}
	...
	return (
		<div className={styles.listContainer}>
			...
			{!isLoading && currentResources.length === 0 ? (
				emptyListComponent ? (
					<div className={styles.emptyListElement}>{emptyListComponent}</div>
				) : (
