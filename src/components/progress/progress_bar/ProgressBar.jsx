...
export const ProgressBar = ({
	...
	const percentage = Number(
		(
			((goal === 0 ? 0 : progress || 0) / (goal === 0 ? 1 : goal)) *
			100
		).toFixed(1),
	);
		...
		<div className={styles.progressContainer}>
			...
			<div className={cx(styles.progressBarContainer, className)}>
				<div
					...
					style={{
						width: percentage >= 100 || barFull ? "100%" : percentage + "%",
					}}
					role="progressbar"
					aria-valuemin="0"
					aria-valuenow={percentage >= 100 || barFull ? 100 : percentage}
					aria-valuemax={"100"}
				...
				>
					<div className={cx(styles.progressBar, innerClassName)}>
						<div className={styles.progressBarContent}>{children}</div>
					</div>
				</div>
			</div>
		</div>
