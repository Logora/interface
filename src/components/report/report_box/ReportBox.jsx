...
export const ReportBox = ({ report, flash }) => {
		...
		switch (reportable_type) {
			case "Message": {
				const shouldHideContent =
					...
					report.reportable?.status === "rejected";

				return (
					<Argument
						argument={reportable}
						positions={
							reportable.group?.group_context?.positions?.slice(0, 2) || []
						}
						hideReplies={true}
						hideFooter={true}
				...
				return (
					...
					>
						{intl.formatMessage({
							...
							defaultMessage:
								"Thank you for your report. After verification, the content has been removed.",
						})}
					</div>
				);
			} else {
				return (
					<div className={styles.statusMessage}>
						{intl.formatMessage({
							...
							defaultMessage:
								"Thank you for your report. After verification, we have not removed the content.",
						})}
					</div>
				);
			}
	...
	return (
		<div
			id={`reports_${report.id}`}
			className={cx(styles.reportBox, {
				[styles.reportHighlighted]: isFlashing,
			})}
		>
			{report && (
					...
					<div className={styles.reportHeader}>
						{report.classification && (
							<div className={styles.reportReason}>
								...
								{intl.messages[
									`report.content_header.moderation_reason.${report.classification.toLowerCase()}`
								]
									? intl.formatMessage({
