...
const reportArgumentPending = generateReportPending({
	reportable_type: "Message",
});
...
describe("ReportBox", () => {
	...
	it("should render Argument component for Message type with correct props", () => {
		renderReportBox({ report: reportArgument });
		expect(
			screen.getByText(reportArgument.reportable.content),
		).toBeInTheDocument();
		expect(
			screen.getByText(reportArgument.reportable.author.full_name),
		).toBeInTheDocument();
	});
	...
	it("should render ProposalBox component for Proposal type with correct props", () => {
		...
		expect(
			screen.getByText(reportProposal.reportable.content),
		).toBeInTheDocument();
		expect(
			screen.getByText(reportProposal.reportable.title),
		).toBeInTheDocument();
	});
	...
	it("should render SuggestionBox component for Group type with correct props", () => {
		...
		expect(
			screen.getByText(reportSuggestion.reportable.name),
		).toBeInTheDocument();
		expect(
			screen.getByText(
				reportSuggestion.reportable.debate_suggestion.author.full_name,
			),
		).toBeInTheDocument();
	});
	...
	it("should display status message for pending status", () => {
		...
		expect(
			screen.getByText(/The report is currently in process/i),
		).toBeInTheDocument();
	});
	...
	it("should display status message for rejected status", () => {
		...
		expect(screen.getByTestId("report-status-message")).toHaveTextContent(
