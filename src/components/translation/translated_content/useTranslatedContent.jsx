...
export const getTranslatedContent = (
	...
	const isValidTranslation = (translationEntry) => {
		return (
			translationEntry.target_language?.substring(0, 2) ===
	...
	if (
		locale?.substring(0, 2) !== originalLanguage?.substring(0, 2) &&
		translations?.length > 0
	) {
		const currentTranslatedContent = translations.filter((t) =>
			isValidTranslation(t),
		)[0];
