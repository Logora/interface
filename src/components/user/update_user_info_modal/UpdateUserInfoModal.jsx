...
export const UpdateUserInfoModal = ({
	...
	const avatarUrlList = [...Array(config.avatars?.maxFileName || 0).keys()].map(
		(j) =>
	...
	const displayPreview = (uri) => {
		...
		reader.onloadend = () => {
			setPreviewPictureBase64(reader.result);
		};
	...
	const submit = (event) => {
		...
		if (showEmailConsent) {
			data["accepts_provider_email"] = acceptsProviderEmail;
		}
		if (showTerms) {
			data["accepts_terms"] = acceptsTerms;
		}
		...
		if (validate(data, validationRules)) {
			...
			if (pendingAuth && onConsentConfirmed) {
				...
				onConsentConfirmed(finalAuthParams);
			} else {
						...
						<div className={styles.toggle}>
							<Toggle
								...
								label={intl.formatMessage(
									...
									{ provider: config.provider?.companyName },
								)}
								onInputChanged={(e) =>
									setAcceptsProviderEmail(!acceptsProviderEmail)
								}
