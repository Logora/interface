...
export const SuggestionVoteBox = ({ voteableType, voteableId, totalUpvote = 0, totalDownvote = 0, onVote, disabled = false }) => {
    ...
    if (isLoggedIn) {
      showModal(
        <Suspense fallback={null}>
          <ConfirmModal
            title={intl.formatMessage({ id: "vote.suggestion_vote_box.downvote", defaultMessage: "Not interesting" })}
  ...
  return (
    <div className={styles.voteButtonsContainer}>
      ...
      >
        <span className={styles.text}>
          {intl.formatMessage({ id: "vote.suggestion_vote_box.downvote", defaultMessage: "Not interesting" })}
        </span>
      </Button>
    </div>
  );
