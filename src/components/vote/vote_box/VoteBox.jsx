...
export const VoteBox = ({
	...
	const firstPosition = useTranslatedContent(
		...
		votePositions[0]?.language,
		"name",
		votePositions[0]?.translation_entries,
	);
	const secondPosition = useTranslatedContent(
		votePositions[1]?.name,
		votePositions[1]?.language,
		"name",
		votePositions[1]?.translation_entries,
	);
	const neutralPosition = useTranslatedContent(
		votePositions[2]?.name,
		votePositions[2]?.language,
		"name",
		votePositions[2]?.translation_entries,
	);
	const [savedVote, setSavedVote, removeSavedVote] = useSessionStorageState(
		"storedUserVote",
		{},
	);
	...
	const getStoredVote = () => {
		...
		const positionId = Number.parseInt(urlParams.get("positionId")) || false;
	...
	const getVote = (debateId) => {
		...
		api
			.getOneWithToken("votes", `${voteableType.toLowerCase()}/${debateId}`, {})
			.then((response) => {
	...
	const addVote = (positionId) => {
		...
		votePositions.forEach((element) => {
			if (positionId === element.id) {
				newVotesCount[element.id] = {
					count: newCount,
					percentage: getPercentageValue(newCount, newTotal),
				};
			} else {
				const newElementCount = votesCount[element.id].count;
				newVotesCount[element.id] = {
					count: newElementCount,
					percentage: getPercentageValue(newElementCount, newTotal),
				};
			}
		});
			...
			const newVotesCount = {
				...
				[previousPosition]: {
					...
					percentage: getPercentageValue(
						votesCount[previousPosition].count - 1,
						totalVotes,
					),
				},
				[newPosition]: {
					...
					percentage: getPercentageValue(
						votesCount[newPosition].count + 1,
						totalVotes,
					),
				},
			};
	...
	const displayVotePosition = (position, index) => {
		...
		if (index === 0) {
			currentPositionName = firstPosition.translatedContent;
		} else if (index === 1) {
			currentPositionName = secondPosition.translatedContent;
		} else if (index === 2) {
			currentPositionName = neutralPosition.translatedContent;
		}
		...
		return (
			<div
				key={index}
				className={cx(styles.voteAction, {
					[styles.buttonAlignLeft]: styles.buttonAlignLeft,
				})}
			>
				<Button
				...
				>
					<div
						data-tid={"action_vote"}
						className={cx(styles.voteButtonThesis, {
							[styles.voteButtonThesisSynthesisMobile]: !displayColumn,
						})}
					>
						{currentPositionName}
					</div>
	...
	return (
		<div
			className={cx(styles.voteBox, { [voteBoxClassName]: voteBoxClassName })}
		>
			<>
							...
							{votePositions.map((value, index) => {
								...
								return (
									<div key={index}>
										...
										>
											{currentPositionName}{" "}
											{currentVote && currentVote.position_id === value.id ? (
												<span
													title={
														intl.formatMessage({
													...
													aria-hidden="true"
												>
													<Icon name="checkbox" width={16} height={16} />
												</span>
											) : null}
										</div>
										<ProgressBar
					...
					<div className={styles.voteBoxActions}>
						<div
							className={cx(styles.voteBoxActionsBody, {
								...
								[styles.voteBoxActionsBodyFilledNeutral]: config.layout?.filledNeutralVoteButton,
						...
						<div
							className={cx(
								styles.voteBoxActions,
								styles.voteBoxShowResultContainer,
								{ [showResultClassName]: showResultClassName },
							)}
						>
